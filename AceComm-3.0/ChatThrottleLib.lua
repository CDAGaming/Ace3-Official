...
function ChatThrottleLib:Init()
	...
	if not self.securelyHooked then
		...
		if _G.C_ChatInfo and _G.C_ChatInfo.SendChatMessage then
			hooksecurefunc(_G.C_ChatInfo, "SendChatMessage", vararg(0, function(arg)
	...
	if not self.securelyHookedLogged then
		...
		if _G.C_ChatInfo then
			hooksecurefunc(_G.C_ChatInfo, "SendAddonMessageLogged", vararg(0, function(arg)
				return ChatThrottleLib.Hook_SendAddonMessageLogged(unpack(arg))
			end))
		end
	end
